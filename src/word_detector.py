...
def detect(
    img: np.ndarray, kernel_size: int, sigma: float, theta: float, min_area: int
) -> List[DetectorRes]:
    ...
    img_thres = (
        255
        - cv2.threshold(img_filtered, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
    )
...
def _cluster_lines(
    ...
    res = sorted(
        clustered.values(),
        key=lambda line: [det.bbox.y + det.bbox.h / 2 for det in line],
    )
    return res
...
def get_word_spacing_features(path: str, img_height: int = 50, theta: float = 5, sigma: float = 11, kernel_size: int = 25, min_area: int = 100, show_intermediary: bool = False) -> Tuple[float, float, float]:

    # load image and process it
    img = prepare_img(cv2.imread(path), img_height)
    if show_intermediary:
        from omar.word_detector import _compute_kernel
        kernel = _compute_kernel(kernel_size, sigma, theta)
        img_blurred = cv2.filter2D(img, -1, kernel, borderType=cv2.BORDER_REPLICATE).astype('uint8')
        plt.figure()
    ...
    for line_idx, line in enumerate(lines):
        for word_idx, det in enumerate(line):
            ...
            #print("Line {}, Word {}: BBox {}".format(line_idx, word_idx, det.bbox))
            if word_idx > 0:
                spacing = det.bbox.x - (line[word_idx - 1].bbox.x + line[word_idx - 1].bbox.w)
                words_spacing.append(spacing)
            #plt.plot(xs, ys, c=colors(line_idx % num_colors))
            #plt.text(det.bbox.x, det.bbox.y, f'{line_idx}/{word_idx}')
    word_spacing_avg = sum(words_spacing) / len(words_spacing) if words_spacing else 0
    word_spacing_stdev = (sum((s - word_spacing_avg) ** 2 for s in words_spacing) / len(words_spacing)) ** 0.5 if words_spacing else 0
    word_spacing_median = sorted(words_spacing)[len(words_spacing) // 2] if words_spacing else 0

    return word_spacing_avg, word_spacing_median, word_spacing_stdev
